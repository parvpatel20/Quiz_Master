  ...
  if (quizzes.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 px-6 py-14 text-center">
        ...
        <p className="font-medium text-fg">No quizzes taken yet</p>
    ...
    <div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[480px] text-sm">
          ...
          <tbody>
            {slice.map((q, i) => (
              <tr key={i} className="border-b border-line transition-colors hover:bg-surface2">
                <td className="px-5 py-3.5 font-medium text-fg">{q.quiz?.subject}</td>
                <td className="px-5 py-3.5 text-muted">{q.quiz?.topic}</td>
                <td className="px-5 py-3.5">
                  <span className={cx("inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold", scoreTone(q.score))}>
                    {q.score}%
                  </span>
                </td>
                <td className="px-5 py-3.5 text-muted">{formatDate(q.quizDate)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 px-5 py-4">
          <button
            ...
            disabled={safePage === 1}
            className="grid h-9 w-9 place-items-center rounded-lg border border-line text-muted transition-colors hover:border-brand/50 hover:text-fg disabled:opacity-40"
          >
