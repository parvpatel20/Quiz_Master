...
const QuizSearchPage = () => {
  ...
  const SelectField = ({
    ...
    <div className="relative group">
      <div
        className={`relative p-6 rounded-3xl shadow-2xl border transition-all duration-500 ${gradient} border-[#FF9100]/30 backdrop-blur-lg`}
      >
        {/* Animated background */}
          ...
          >
            ...
            {options.map((option) => (
              <option
                key={option.value}
                value={option.value}
                className="bg-[#1a2845] text-white"
              >
                {option.label}
              </option>
            ))}
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
            <SelectField
              ...
              options={Object.keys(data).map((cls) => ({
                value: cls,
                label: cls,
              }))}
